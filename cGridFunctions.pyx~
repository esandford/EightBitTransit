...
def pixeloverlaparea(x0,y0,w):
	...
	if rsq > ((np.sqrt(2.)*w)):
		...
		xarr = np.zeros(8)
		xarr[0] = np.sqrt((1. - 0.5*w - y0)*(1. + 0.5*w + y0))
		xarr[1] = -xarr[0]
		
		xarr[2] = x0 + 0.5*w
		xarr[3] = xarr[2]
		
		xarr[4] = -np.sqrt((1. + 0.5*w - y0)*(1. - 0.5*w + y0))
		xarr[5] = -xarr[4]
		...
		yarr[2] = -np.sqrt((1. - 0.5*w - x0)*(1. + 0.5*w + x0))
		yarr[3] = -yarr[2]
		
		yarr[4] = y0 - 0.5*w
		yarr[5] = yarr[4]
		
		yarr[6] = np.sqrt((1. + 0.5*w - x0)*(1. - 0.5*w + x0))
		yarr[7] = -yarr[6]
		...
		if num_intersections==2: #pixel partially overlaps with stellar disk
			...
			if (vg[0]%2 == vg[1]%2) and (vg[0] != vg[1]): #either the points of intersection are top/bottom sides of pixel, or they are left/right
				...
				#print "triangle"
				area = 0.5*np.abs(xg[1] - xg[0])*np.abs(yg[1] - yg[0])
				chord = np.sqrt((xg[1] - xg[0])**2 + (yg[1] - yg[0])**2)
				h = 1. - np.sqrt(1. - (chord**2/4.)) # 1**2 = (chord/2)**2 + (1-h)**2

				if (np.sqrt(x0**2 + y0**2) < (1. - h)):
			...
			corners = np.zeros(3)
			for i in range(0,3):
				if (np.abs(xg[i]) == 1.0) and (np.abs(yg[i]) == 0.0):
					corners[i] = 1.
			...
			if np.sum(corners)==3.:
				...
				for idx in outeridx:
					triarea = 0.5*np.abs(xg[idx] - xg[middleidx])*np.abs(yg[idx] - yg[middleidx])
				
					chord = np.sqrt((xg[idx] - xg[middleidx])**2 + (yg[idx] - yg[middleidx])**2)
					h = 1. - np.sqrt(1. - (chord**2/4.))
					if ((x0**2 + y0**2) < (1. - h)):
						#most of pixel is inside the circle
						triarea = -1*triarea
			...
			if middlevert%2 == 1: #middle intersection is on top or bottom
				...
				xd00 = np.abs(xg[outeridx0] - xg[middleidx0])
				xd01 = np.abs(xg[outeridx0] - xg[middleidx1])
				
				if xd00 <= xd01:
					triarea0 = 0.5*xd00*np.abs(yg[outeridx0] - yg[middleidx0])
					chord0 = np.sqrt(xd00**2 + (yg[outeridx0] - yg[middleidx0])**2)
					#print triarea0
					#print chord0
					area = area - triarea0 + np.arcsin(0.5*chord0) - 0.5*np.sin(2.*np.arcsin(0.5*chord0))
					
					triarea1 = 0.5*np.abs(xg[outeridx1] - xg[middleidx1])*np.abs(yg[outeridx1] - yg[middleidx1])
					chord1 = np.sqrt((xg[outeridx1] - xg[middleidx1])**2 + (yg[outeridx1] - yg[middleidx1])**2)
					area = area - triarea1 + np.arcsin(0.5*chord1) - 0.5*np.sin(2.*np.arcsin(0.5*chord1))
					#print triarea1
					#print chord1
				else:
					triarea0 = 0.5*xd01*np.abs(yg[outeridx0] - yg[middleidx1])
					chord0 = np.sqrt(xd01**2 + (yg[outeridx0] - yg[middleidx1])**2)
					area = area - triarea0 + np.arcsin(0.5*chord0) - 0.5*np.sin(2.*np.arcsin(0.5*chord0))
					#print triarea0
					triarea1 = 0.5*np.abs(xg[outeridx1] - xg[middleidx0])*np.abs(yg[outeridx1] - yg[middleidx0])
					chord1 = np.sqrt((xg[outeridx1] - xg[middleidx0])**2 + (yg[outeridx1] - yg[middleidx0])**2)
					area = area - triarea1 + np.arcsin(0.5*chord1) - 0.5*np.sin(2.*np.arcsin(0.5*chord1))
					#print triarea1
				...
				yd00 = np.abs(yg[outeridx0] - yg[middleidx0])
				yd01 = np.abs(yg[outeridx0] - yg[middleidx1])
				
				if yd00 <= yd01:
					triarea0 = 0.5*yd00*np.abs(xg[outeridx0] - xg[middleidx0])
					chord0 = np.sqrt(yd00**2 + (xg[outeridx0] - xg[middleidx0])**2)
					#print triarea0
					area = area - triarea0 + np.arcsin(0.5*chord0) - 0.5*np.sin(2.*np.arcsin(0.5*chord0))
					...
					triarea0 = 0.5*yd01*np.abs(xg[outeridx0] - xg[middleidx1])
					chord0 = np.sqrt(yd01**2 + (xg[outeridx0] - xg[middleidx1])**2)
					area = area - triarea0 + np.arcsin(0.5*chord0) - 0.5*np.sin(2.*np.arcsin(0.5*chord0))
					#print triarea0
					triarea1 = 0.5*np.abs(xg[outeridx1] - xg[middleidx0])*np.abs(yg[outeridx1] - yg[middleidx0])
					chord1 = np.sqrt((xg[outeridx1] - xg[middleidx0])**2 + (yg[outeridx1] - yg[middleidx0])**2)
			...
			non_tb_y = []
			for i in range(0,6):
				if (np.abs(xg[i]) == 0.0) and (np.abs(yg[i]) == 1.0):
					pass
				else:
			...
			chord = np.sqrt((non_tb_x[1] - non_tb_x[0])**2 + (non_tb_y[1] - non_tb_y[0])**2)

			unobscured_area = np.arcsin(0.5*chord) - 0.5*np.sin(2.*np.arcsin(0.5*chord))
			return 1.0 - (float(unobscured_area)/np.pi)
	...
	norm_area = float(area)/np.pi #normalize by stellar area, which is pi*1**2
	return norm_area
...
def LDfluxsmall(x, y, t, Ar_occ, c1, c2, c3, c4, w):
	...
	r = w/np.sqrt(np.pi) #set r such that area = w^2= pi*r^2 => r = w/sqrt(pi), i.e. r is the radius of the circle with area w**2
	...
	for i in range(0,n):
		...
		if S[i] > (1. + r): #case I: pixel is outside of stellar disk
			Ar_ann[i] = 0.
			Fl_ann[i] = 0.
			I0[i] = 0.
		
		elif (S[i] > r) and (S[i] < (1.-r)): #case III: pixel fully overlaps stellar disk
			Ar_ann[i] = np.pi*(bm[i] - am[i])
			Fl_ann[i] = (am[i] - bm[i])*(c1 + c2 + c3 + c4 - 1.) + 0.8*c1*amR[i]**5 + (2./3.)*c2*amR[i]**6 + (4./7.)*c3*amR[i]**7 + 0.5*c4*amR[i]**8 - 0.8*c1*bmR[i]**5 - (2./3.)*c2*bmR[i]**6 -(4./7.)*c3*bmR[i]**7 - 0.5*c4*bmR[i]**8
			I0[i] = np.pi*(Ar_occ[i]/Ar_ann[i])*(Fl_ann[i]/Ftot)
		
		elif (S[i] < r): #case IV: pixel is very close to stellar center
			#if np.isnan(bmR[i]):
				#print "case IV problem! bmR is nan"

			Ar_ann[i] = np.pi*bm[i]		
			Fl_ann[i] = -bm[i]*(c1 + c2 + c3 + c4 - 1.) + 0.8*c1 + (2./3.)*c2 + (4./7.)*c3 + 0.5*c4 -0.8*c1*bmR[i]**5 - (2./3.)*c2*bmR[i]**6 - (4./7.)*c3*bmR[i]**7 - 0.5*c4*bmR[i]**8
			I0[i] = np.pi*(Ar_occ[i]/Ar_ann[i])*(Fl_ann[i]/Ftot)
			...
			Ar_ann[i] = np.pi*(1.-am[i])
			Fl_ann[i] = (am[i] - 1.)*(c1 + c2 + c3 + c4 - 1.) + 0.8*c1*amR[i]**5 + (2./3.)*c2*amR[i]**6 +(4./7.)*c3*amR[i]**7 + 0.5*c4*amR[i]**8
			I0[i] = np.pi*(Ar_occ[i]/Ar_ann[i])*(Fl_ann[i]/Ftot)
